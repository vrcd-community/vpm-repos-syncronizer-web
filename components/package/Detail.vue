    ...
    <div class="flex-1">
      <Tabs v-model:value="tabModel">
        <TabList>
          ...
          <Tab
            v-if="selectedPackageVersion.samples && selectedPackageVersion.samples.length != 0"
            value="samples"
          >
            包中的示例
          </Tab>
        ...
        <TabPanels>
          ...
          <TabPanel value="dependencies">
            <div class="space-y-4">
              <PackageDependenceList
                type="UPM"
                :dependencies="selectedPackageVersion.dependencies"
              />
              <PackageDependenceList
                type="VPM"
                :dependencies="selectedPackageVersion.vpmDependencies"
              />
              <PackageDependenceList
                type="Git"
                :dependencies="selectedPackageVersion.gitDependencies"
              />
            </div>
          </TabPanel>
          <TabPanel value="samples">
            <ul class="space-y-4">
              <li
                v-for="sample in selectedPackageVersion.samples"
                :key="sample.displayName+sample.path"
              >
          ...
          <TabPanel value="versions">
            <NuxtLink
              ...
              :to="version.name + '@' + version.version"
            >
              <i
                v-if="version.version === selectedPackageVersion.version"
                class="pi pi-check text-xs"
              />
              <span>{{ version.version }}</span>
              <div class="*:text-xs *:font-medium *:whitespace-nowrap flex items-center space-x-2">
                <Tag
                  v-if="version.version === package.latest.version"
                  severity="success"
                  value="最新版本"
                  icon="pi pi-star"
                />
                <Tag
                  v-if="version.license"
                  severity="info"
                >
    ...
    <div class="w-64 space-y-4">
      <di v-if="typeof selectedPackageVersion.changelogUrl === 'string'">
        ...
        <div class="flex items-center gap-1">
          ...
          <a
            class="flex-1 text-primary underline text-ellipsis overflow-hidden"
            :href="selectedPackageVersion.changelogUrl"
            target="_blank"
          >{{
      ...
      <div v-if="selectedPackageVersion.author">
        ...
        <ul>
          <li
            v-if="selectedPackageVersion.author.email"
          ...
          <li
            v-if="selectedPackageVersion.author.url"
            class="flex items-center gap-1"
          >
            <IconEarth class="size-4" />
            <a
              class="flex-1 text-primary underline text-ellipsis overflow-hidden"
              :href="selectedPackageVersion.author.url"
              target="_blank"
            >{{
