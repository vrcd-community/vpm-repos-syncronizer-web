...
<template>
  ...
  <div v-if="task" class="mb-4 space-y-3">
    <div class="flex items-baseline space-x-2">
      <SyncTaskStatusIcon :status="task.status" />
      <h1 class="text-3xl">{{ task.repoId }}</h1>
      <span class="text-xl opacity-60">#{{ task.id }}</span>
    </div>
    <div v-if="repo" class="grid grid-cols-2 gap-4">
      <div>
        <h3>上游地址</h3>
        <TextCopy :text="repo.upstreamUrl" tooltip="复制上游地址">复制上游地址</TextCopy>
      </div>
      <div>
        <h3>镜像地址</h3>
        <TextCopy :text="repo.repoUrl" tooltip="复制镜像地址">复制镜像地址</TextCopy>
      </div>
      <div>
        <h3>同步开始时间</h3>
        <NuxtTime :datetime="task.startTime" dateStyle="full" time-style="long" />
      </div>
      <div>
        <h3>同步结束时间</h3>
        <NuxtTime v-if="task.endTime" :datetime="task.endTime" dateStyle="full" time-style="long" />
        <p v-else>同步正在进行</p>
      </div>
    </div>
    <h2 class="font-semibold text-xl">任务日志</h2>
